...
5. `05-transactions.sql` — transactions. **New thing**: statement-level vs transaction-level error behaviour. See `05-transactions.md` for a walkthrough of `BEGIN`/`COMMIT`/`ROLLBACK`, the whole-transaction abort rule, and `SAVEPOINT`.
